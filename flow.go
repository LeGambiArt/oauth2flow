...
//
// After tokens are acquired and saved, the consuming application handles
...
// Compatible with Google's Python oauth2client format and
// what-the-mcp-ng's OAuth2Provider.
...
// (e.g., downloaded from Google Cloud Console) and returns an
// oauth2.Config. Supports both "installed" and "web" client types.
func LoadClientCredentials(path string, scopes []string) (*oauth2.Config, error) {
	return loadClientCredentials(path, scopes)
}
